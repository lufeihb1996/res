...
第 2 层：页面怎么变
  state、事件、条件渲染、列表渲染、表单

第 3 层：数据怎么来
  useEffect、fetch、loading、error
...
| 顺序 | 学什么 | 学到什么程度算过关 |
|---:|---|---|
| 1 | JSX | 能看懂 `{}`、`className`、组件返回 UI |
| 2 | 组件 | 能写 `function App() { return ... }` |
| 3 | props | 能让父组件给子组件传数据 |
| 4 | useState | 能做计数器、开关、购物车数量 |
| 5 | 事件 | 能处理按钮点击、输入框输入 |
| 6 | 条件渲染 | 能根据状态显示/隐藏内容 |
| 7 | 列表渲染 | 能用 `map` 渲染数组，并写 `key` |
| 8 | 表单 | 能写 `value + onChange` |
| 9 | useEffect | 能在页面加载时请求数据 |
| 10 | 组件拆分 | 能把一个大页面拆成多个小组件 |
| 11 | 父子通信 | 能让子组件通知父组件 |
| 12 | Context / 自定义 Hook | 能复用全局数据和逻辑 |
| 13 | 路由 / Next.js | 能做多页面、SEO、后端 API |

最重要的是前 8 个。前 8 个学会，就能开始写普通 React 页面。
...
先把 React 组件和 state 搞清楚，后面学 Next.js 才不会乱。
